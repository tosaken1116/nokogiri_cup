...
export const Animal = (props: AnimalProps)=>{
    ...
    let animalImage: string = eggImages[0];
    let imageScale = 1;
    let imageWid: number;
    let imageHi: number;
    if(props.articleAmount < 12){
        animalImage = eggImages[Math.floor(props.articleAmount/2)];
        imageWid = 48;
        imageHi = 56;
    }
    else if(props.articleAmount < 16){
        if(isStop)
        ...
        imageWid = 48 + 48 * (props.articleAmount-11)/4;
        imageHi = 56 + 56 * (props.articleAmount-11)/4;
    }
    else{
        if(isStop)
